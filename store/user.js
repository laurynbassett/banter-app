...
// ADD NEW CONTACT
export const addNewContact = ({ email }, navigation) => async (dispatch, getState) => {
  ...
  try {
    ...
    if (email) {
      snapshot = await usersRef.orderByChild("email").equalTo(email.toLowerCase()).once("value");
    }
...
export const registerForPushNotificationsAsync = () => async (dispatch, getState) => {
  ...
  try {
    ...
    if (Constants.isDevice) {
      ...
      await firebase.database().ref(`/users/${uid}/notifications/`).update({ token: token });
